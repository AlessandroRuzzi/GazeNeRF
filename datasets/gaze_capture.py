...
def get_train_loader(
    ...
    # load dataset
    refer_list_file = os.path.join("data/gaze_capture", "train_test_split.json")
...
class GazeDataset(Dataset):
    def __init__(
        ...
        self.target_idx = np.loadtxt("configs/config_files/gaze_capture_evaluation_target_single_subject.txt", dtype=np.int)
    ...
    def __getitem__(self, idx_input):
        ...
        self.hdf = h5py.File(
            os.path.join(self.path,"gaze_capture_" +  self.selected_keys[key]),
            "r",
            swmr=True,
        )
